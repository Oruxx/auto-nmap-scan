...
if [ "$2" == "All" ]; then
	...
	echo -e "${YELLOW}Running a $2 scan on $ip"
fi
...
reconRecommend(){
  ...
  if [ -f nmap/UDP_"$ip".nmap ] && [[ ! -z $(cat nmap/UDP_"$ip".nmap | grep open | grep -w "161/udp") ]]; then
    ...
    echo "snmpwalk -Os -c public -v1 $ip | tee recon/snmpwalk_$ip.txt"
    echo ""
  fi
  ...
  if [[ ! -z $(echo "${file}" | grep -w "53/tcp") ]]; then
    ...
    echo "dig -x $ip @$ip | tee recon/dig_$ip.txt"
    echo ""
  fi
  ...
  if [[ ! -z $(echo "${file}" | grep -w "389/tcp") ]]; then
    ...
    echo -e "${NC}"
    echo "ldapsearch -x -h $ip -s base | tee recon/ldapsearch_$ip.txt"
    echo "ldapsearch -x -h $ip -b \$(cat recon/ldapsearch_$ip.txt | grep rootDomainNamingContext | cut -d ' ' -f2) | tee recon/ldapsearch_DC_$ip.txt"
    echo "nmap -Pn -p 389 --script ldap-search --script-args 'ldap.username=\"\$(cat recon/ldapsearch_$ip.txt | grep rootDomainNamingContext | cut -d \\" \\" -f2)\"' $ip -oN recon/nmap_ldap_$ip.txt"
    echo ""
  fi
...
runRecon(){
  ...
  if [ "$2" == "All" ]; then
    ...
    reconCommands=$(cat nmap/Recon_"$ip".nmap | grep "$ip" | grep "$2")
  fi
...
if [[ $1 =~ ^[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+$ ]]; then
  ...
  for i in $(cat $1);do
    ...
    case "$2" in
      Quick | quick) 	quickScan "$ip";;
      Basic | basic)	if [ ! -f nmap/Quick_"$ip".nmap ]; then quickScan "$ip"; fi
          basicScan "$ip";;
      UDP | udp) 	UDPScan "$ip";;
      Full | full) 	fullScan "$ip";;
      Vulns | vulns) 	if [ ! -f nmap/Quick_"$ip".nmap ]; then quickScan "$ip"; fi
          vulnsScan "$ip";;
      Recon | recon) 	if [ ! -f nmap/Quick_"$ip".nmap ]; then quickScan "$ip"; fi
          if [ ! -f nmap/Basic_"$ip".nmap ]; then basicScan "$ip"; fi
          recon "$ip";;
      All | all)	quickScan "$ip"
          basicScan "$ip"
          UDPScan "$ip"
          fullScan "$ip"
          vulnsScan "$ip"
...
#echo modificar el flujo para insertar nuevos valores
